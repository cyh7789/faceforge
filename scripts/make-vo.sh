...
write 03 "Two players, one phone, side by side. A local face detector confirms each face before anything leaves the device. Then the YouCam Skin Analysis API scores fifteen skin metrics for each player."
...
write 09 "Everything here is built on measured API behavior. Bad photos cost seventy-eight seconds of silent retries, so the face gate stops them locally and saves credits. The flattering UI scores hide the interesting signal, so the whole engine runs on raw scores."
